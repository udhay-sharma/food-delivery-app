...
* **Interactive Design & Architecture Canvas**: [TLDraw Workspace Architecture Canvas](https://www.tldraw.com/f/s-Zto03AvDk7C2ePL4Cbc?d=v-936.985.3506.2295.page)
...
4. **Pure Conditional Routing**: Root navigation between Onboarding, Login, and the Main App flows is managed purely via reactive state synchronization (`isOnboarded` and `isLoggedIn`). Component-level manual transition attempts (such as `navigation.replace`) are fully avoided to eliminate navigation clashes, allowing React Navigation to cleanly mount and unmount screen blocks without race conditions.
